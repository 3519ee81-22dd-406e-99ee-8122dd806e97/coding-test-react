...
export const getCategoryTotals = (products: Product[]): CategorySummary => {
  return products.reduce((acc, product) => {
    acc[product.category] = {
      totalPrice: (acc[product.category]?.totalPrice || 0) + product.price,
    };
    return acc;
  }, {} as CategorySummary);
};
...
export const getDepartmentSummary = (users: User[]): DepartmentSummary => {
  return users.reduce((acc, user) => {
    ...
    dept.userCount++;
    const currentTotal = dept.averageAge * (dept.userCount - 1) + user.age;
    dept.averageAge = currentTotal / dept.userCount;
    return acc;
