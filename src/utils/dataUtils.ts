...
export const getCategoryTotals = (products: Product[]): CategorySummary => {
  return products.reduce((acc, product) => {
    ...
    acc[product.category].totalPrice += totalPrice;
...
export const getDepartmentSummary = (users: User[]): DepartmentSummary => {
  ...
  users.forEach(user => {
    ...
    summary[dept].userCount += 1;
    summary[dept].averageAge += user.age;
  });

  for (const dept in summary){
    summary[dept].averageAge = summary[dept].averageAge / summary[dept].userCount;
  }
