...
export const getCategoryTotals = (products: Product[]): CategorySummary => {
  return products.reduce((sum, product) => {
    if (!sum[product.category]) sum[product.category] = { totalPrice: 0 };

    sum[product.category].totalPrice += product.price * product.stock;
    return sum;
  }, {} as CategorySummary);
};
...
export const getDepartmentSummary = (users: User[]): DepartmentSummary => {
  return users.reduce((sum, user) => {
    ...
    const dept = sum[user.department];
    const total = dept.averageAge * dept.userCount;
    dept.userCount += 1;
    dept.averageAge = (total + user.age) / dept.userCount;
