...
const UserProfile: React.FC<UserProfileProps> = ({ user, stats, posts }) => {
  return (
    <div className={styles.profileContainer}>
      {/* 1. 프로필 헤더 */}
      <ProfileHeader id={user.id} name={user.name} username={user.username} bio={user.bio} avatarUrl={user.avatarUrl}  />

      {/* 2. 사용자 정보 */}
      <UserInfo name={user.name} bio={user.bio} />

      {/* 3. 사용자 통계 */}
      <UserStat posts={stats.posts} followers={stats.followers} following={stats.following} />

      {/* 4. 게시물 그리드 */}
      <UserGrid imageUrl={posts.imageUrl} />
    </div>
  );
};
